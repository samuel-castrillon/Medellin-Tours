<?php
    ...
    */
    ...
    function create($p_user_name, $p_user_lastname, $p_user_email, $p_user_phone, $p_user_birthdate, $p_user_city, $p_user_password, $p_conection){
        $sql = "INSERT INTO usuarios (nombre, apellido, email, telefono, fecha_nacimiento, ciudad_residencia, contraseña) VALUES (:u_name, :lastname, :email, :phone, :birthdate, :city, :u_password)";

        $query = $p_conection->prepare($sql);
        $query->bindParam(":u_name", $p_user_name);
        ...
        $query->bindParam(":u_password", $p_user_password);

        $query->execute();

        $registered = $p_conection->lastInsertId();
        if($registered > 0){
            echo "Se hizo el registro exitosamente. ID: " . $registered;
        } else {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        create($user_name, $user_lastname, $user_email, $user_phone, $user_birthdate, $user_city, $user_password, $conexion);
    }    
?>